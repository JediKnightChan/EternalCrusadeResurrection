...
void UECRCameraComponent::OnRegister()
{
	...
	if (!CameraModeStack)
	{
		CameraModeStack = NewObject<UECRCameraModeStack>(this);
...
void UECRCameraComponent::UpdateCameraModes()
{
	...
	if (CameraModeStack->IsStackActivate())
	{
		...
		{
			if (const TSubclassOf<UECRCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
			{
...
void UECRCameraComponent::DrawDebug(UCanvas* Canvas) const
{
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("ECRCameraComponent: %s"), *GetNameSafe(GetTargetActor())));
