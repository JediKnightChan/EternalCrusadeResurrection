...
{
	return CastChecked<UECRCameraComponent>(GetOuter());
}
...
void UECRCameraMode::SetBlendWeight(float Weight)
{
	...
	{
	case EECRCameraModeBlendFunction::Linear:
		BlendAlpha = BlendWeight;
		break;

	case EECRCameraModeBlendFunction::EaseIn:
		BlendAlpha = FMath::InterpEaseIn(0.0f, 1.0f, BlendWeight, InvExponent);
		break;

	case EECRCameraModeBlendFunction::EaseOut:
		BlendAlpha = FMath::InterpEaseOut(0.0f, 1.0f, BlendWeight, InvExponent);
		break;

	case EECRCameraModeBlendFunction::EaseInOut:
...
void UECRCameraModeStack::ActivateStack()
{
	if (!bIsActive)
	{
		...
		for (UECRCameraMode* CameraMode : CameraModeStack)
		{
			check(CameraMode);
...
void UECRCameraModeStack::PushCameraMode(TSubclassOf<UECRCameraMode> CameraModeClass)
{
	...
	UECRCameraMode* CameraMode = GetCameraModeInstance(CameraModeClass);
	check(CameraMode);
...
UECRCameraMode* UECRCameraModeStack::GetCameraModeInstance(TSubclassOf<UECRCameraMode> CameraModeClass)
{
	...
	for (UECRCameraMode* CameraMode : CameraModeInstances)
	{
	...
	UECRCameraMode* NewCameraMode = NewObject<UECRCameraMode>(GetOuter(), CameraModeClass, NAME_None, RF_NoFlags);
	check(NewCameraMode);
...
void UECRCameraModeStack::UpdateStack(float DeltaTime)
{
	...
	for (int32 StackIndex = 0; StackIndex < StackSize; ++StackIndex)
	{
		UECRCameraMode* CameraMode = CameraModeStack[StackIndex];
		check(CameraMode);
...
void UECRCameraModeStack::BlendStack(FECRCameraModeView& OutCameraModeView) const
{
	...
	const UECRCameraMode* CameraMode = CameraModeStack[StackSize - 1];
	check(CameraMode);
...
void UECRCameraModeStack::DrawDebug(UCanvas* Canvas) const
{
	...
	for (const UECRCameraMode* CameraMode : CameraModeStack)
	{
		check(CameraMode);
...
void UECRCameraModeStack::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	...
	{
		UECRCameraMode* TopEntry = CameraModeStack.Last();
		check(TopEntry);
